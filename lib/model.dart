...
abstract class CrosswordPuzzleGame
  ...
  factory CrosswordPuzzleGame.from({
    ...
    for (final crosswordWord in crossword.words) {
      // Obtener palabras del mismo tamaño
      final sameLengthWords = candidateWords
          .where((word) => word.length == crosswordWord.word.length)
          .toList();
      
      // Si hay suficientes palabras, tomar más alternativas (10-15)
      // Si no hay suficientes, tomar todas las disponibles
      final numAlternates = sameLengthWords.length >= 15 
          ? 15 
          : (sameLengthWords.length >= 10 ? 10 : sameLengthWords.length);
      
      // Mezclar y tomar las palabras alternativas
      sameLengthWords.shuffle();
      final alternateWords = sameLengthWords
          .take(numAlternates > 0 ? numAlternates : 1)
          .toList()
        ..sort();
      
      final alternateWordsList = alternateWords.toBuiltList();

      // Remover las palabras alternativas seleccionadas para evitar duplicados
      candidateWords = candidateWords.rebuild(
        (b) => b.removeAll(alternateWordsList),
      );
        ...
        (b) => b.updateValue(
          ...
          (b) => b.rebuild(
            (b) => b.updateValue(
              ...
              (b) => b.rebuild((b) => b.replace(alternateWordsList)),
              ifAbsent: () => alternateWordsList,
            ),
          ),
          ifAbsent: () => {crosswordWord.direction: alternateWordsList}.build(),
        ),
      );
    }
